...
class SerializeFieldController(
    ...
    fieldType match {
      case MapType(k, v, _) => {
        Map(
          ...
          "serialize_key" -> indent(generator.serializeField(k, tmp_iter + ".Key", ns), 8),
          "serialize_val" -> indent(generator.serializeField(v, tmp_iter + ".Value", ns), 8)
        )
      }
      case _ => false
    }
  ...
  def deserialize_elem(x: FieldType): Map[String, Object] = {
    Map(
      ...
      "serialize_elem" -> indent(generator.serializeField(x, tmp_iter.toString, ns), 4)
    )
  }
}
