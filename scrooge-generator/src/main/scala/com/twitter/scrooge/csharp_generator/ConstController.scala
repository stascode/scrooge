 ...
 */
class ConstController(defs: Seq[ConstDefinition], generator: CsharpGenerator, ns: Option[Identifier])
  extends TypeController("Constants", generator, ns) {
  val constants = defs map { d =>
    Map("rendered_value" -> indent(generator.printConstValue(d.sid.name, d.fieldType, d.value, ns), 4))
  }
}
